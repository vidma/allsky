...
int main(int argc, char *argv[])
{
    ...
    char id[32];
    char camtype[16];
    unsigned int width,height,bpp,channels;
	long ltemp     = 0;
    ...
    ret = GetQHYCCDChipInfo(camhandle,&chipw,&chiph,&width,&height,&pixelw,&pixelh,&bpp);
    if(ret == QHYCCD_SUCCESS)
    {
	...
	SetQHYCCDParam(camhandle, QHYCCD_3A_AUTOEXPOSURE, 1);
	SetQHYCCDParam(camhandle, CONTROL_EXPOSURE, 5000000);
    ...
    ret = SetQHYCCDResolution(camhandle,0,0,width,height);
    if(ret == QHYCCD_SUCCESS)
    {
    ...
    while (bMain)
    {
        ...
        if (needCapture)
        {
            ...
            {
                if (GetQHYCCDLiveFrame(camhandle,&width,&height,&bpp,&channels,ImgData) == QHYCCD_SUCCESS)
                {
				autoExp = GetQHYCCDParam(camhandle,CONTROL_EXPOSURE);
                    ...
                    if (darkframe != 1)
                    {
                        // If darkframe mode is off, add overlay text to the image(char *)ImgData;
                        int iYOffset = 0;
